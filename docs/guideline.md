...
`resolvedDeveloperDirs()` covers the Xcode/CLT toolchain. The profile grants `/Library/Developer/CommandLineTools` and `/Applications/Xcode.app` statically, but `/usr/bin/python3` — like `clang`, `git` and `swift` — is a shim that execs whatever `xcode-select -p` currently selects, and Seatbelt matches the **symlink-resolved** path. On a developer mac `Xcode.app` is a real directory and the static grant is enough; on a CI runner the bundle is versioned (`/Applications/Xcode_16.4.app`) with `Xcode.app` a symlink onto it, so the static grant matches nothing and CPython dies at preinit with `_Py_HashRandomization_Init: failed to get random numbers`. The symptom is a functional test that passes on every laptop and fails only in CI. The resolver therefore reads the realpath of `/var/db/xcode_select_link` (the link `xcode-select` maintains — read through `fs`, keeping the module's "no I/O beyond autodetection" rule) plus the realpaths of the static pair, drops anything already granted, and hands the rest to the same read/exec section. Best-effort: an absent or unreadable link contributes nothing, exactly like `resolvedClaboxHome()`.
